...
class CoreStrategyAndMissionView(BaseModel):
    """## 公司的核心战略目标及使命是什么？ [id:core_strategy_and_mission]"""
...
class OrgSupportForStrategyView(BaseModel):
    """## 公司的组织架构是否支持公司的战略达成？ [id:org_support_for_strategy]"""
...
class CollaborationAndConflictView(BaseModel):
    """## 如何保证人与人、人与部门、部门与部门之间的顺畅协同和沟通？ [id:collaboration_and_conflict]"""
...
class BusinessSupportSystemView(BaseModel):
    """## 为了支持业务的顺利进行在有哪些方面的支持？ [id:business_support_system]"""
...
class EmployeeIncentivePolicyView(BaseModel):
    """## 公司对员工的激励政策是什么？ [id:employee_incentive_policy]"""
...
class LeadershipAndOperationsView(BaseModel):
    """## 公司领导如何保证企业整体的有效运作？ [id:leadership_and_operations]"""
...
# 公司核心战略目标及管理情况如何？ [id:company_core_strategy_and_management]
