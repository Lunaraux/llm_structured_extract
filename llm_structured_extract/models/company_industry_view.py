...
class OverallIndustryView(BaseModel):
    """## 整体行业情况如何？（为何过去会这样？未来将会怎么样？以及政策对行业影像情况如何？） [id:overall_industry]"""
...
class SubIndustryView(BaseModel):
    """## 细分行业情况如何？（细分市场过去和未来为何会这样？未来细分市场的占比情况会如何？） [id:sub_industry]"""
...
class IndustryChainView(BaseModel):
    """## 行业产业链情况如何？（产业链的每块成本和利润占比情况如何？未来产业链的变化方向是什么？） [id:industry_chain]"""
...
class IndustryCompetitionView(BaseModel):
    """## 行业竞争情况如何？ [id:industry_competition]"""
    market_competition_dynamics: Optional[str] = Field(
        None,
    ...
    competitor_analysis_detail: Optional[str] = Field(
        None,
...
class CustomerAndConsumerView(BaseModel):
    """## 行业消费者和客户情况如何？（目前消费者和客户是否满意现在的产品和服务？基于消费者的需求未来产品变化的方向和趋势是什么？） [id:customer_and_consumer]"""
...
""".strip()
    ...
    overall_industry: OverallIndustryView = Field(
        ...
        json_schema_extra={"markdown_title": "整体行业情况如何？（为何过去会这样？未来将会怎么样？以及政策对行业影像情况如何？）"}
    )
    sub_industry: SubIndustryView = Field(
        default_factory=SubIndustryView,
        json_schema_extra={"markdown_title": "细分行业情况如何？（细分市场过去和未来为何会这样？未来细分市场的占比情况会如何？）"}
    )
    industry_chain: IndustryChainView = Field(
        default_factory=IndustryChainView,
        json_schema_extra={"markdown_title": "行业产业链情况如何？（产业链的每块成本和利润占比情况如何？未来产业链的变化方向是什么？）"}
    )
    industry_competition: IndustryCompetitionView = Field(
        default_factory=IndustryCompetitionView,
        json_schema_extra={"markdown_title": "行业竞争情况如何？"}
    )
    customer_and_consumer: CustomerAndConsumerView = Field(
        default_factory=CustomerAndConsumerView,
        json_schema_extra={"markdown_title": "行业消费者和客户情况如何？（目前消费者和客户是否满意现在的产品和服务？基于消费者的需求未来产品变化的方向和趋势是什么？）"}
    )